layui.define(["jquery", "layer", "laypage"], function(exports) {
  ...
  var fm = {
      ...
      on: function(events, callback) {
        return layui.onevent.call(this, "fileManager", events, callback);
      },
    ...
    thisMm = function() {
      ...
      // console.log(id)
      if (id) {
        thisMm.that[id] = that; //记录当前实例对象
        thisMm.config[id] = options; //记录当前实例配置项
      }
      return {
        config: options,
    ...
    //获取当前实例配置项
    getthisMmConfig = function(id) {
      var config = thisMm.config[id];
    ...
    Class = function(options) {
      ...
      //记录所有实例
      thisMm.that = {}; //记录所有实例对象
      thisMm.config = {}; //记录所有实例配置项
  ...
  Class.prototype.render = function() {
    ...
    if (options.btn_create) {
      _btn +=
        '<button type="button" class="layui-btn layui-btn-primary layui-btn-sm" id="new_dir">建文件夹</button>';
    }
    if (options.btn_upload) {
      _btn +=
        '<button type="button" class="layui-btn layui-btn-primary layui-btn-sm" id="uploadfile">上传文件</button>';
    }
    var _html =
      ...
      '<ul class="mediaManager layui-row fm_body layui-col-space10" >' +
      "</ul>" +
      "</div>" +
      '<hr><div ><div class="layui_page_' +
      options.id +
      '" id="layui_page_' +
      options.id +
      '"></div></div></div>';
  ...
  Class.prototype.renderData = function(res, curr, count) {
    ...
    layui.each(data, function(i, v) {
      ...
      switch (v.type) {
        case "directory":
          _img =
            ...
            options.thumb["width"] +
            "px;height:" +
            options.thumb["height"] +
            "px;line-height:" +
            options.thumb["height"] +
            'px"><img src="ico/dir.png" style="vertical-align:middle;"></div>';
          _type = "DIR";
          break;
        default:
          if (
      ...
      _content +=
        '<li style="display:inline-block" data-type="' +
        _type +
        '" data-index="' +
        i +
        '">' +
    ...
    options.elem.find(".mediaManager").html(_content);
  ...
  Class.prototype.events = function() {
    ...
    //文件事件
    that.layBody.on("click", "li", function() {
      //单击行
      setPicEvent.call(this, "pic");
    });
    //文件夹事件
    that.layBody.on("click", "li[data-type=DIR]", function() {
      //单击行
      var othis = $(this);
    ...
    that.layToolBar.on("click", "#uploadfile", function() {
      ...
      layui.event.call(this, "mediaManager", eventType + "(" + filter + ")", {
        obj: othis,
    ...
    that.layToolBar.on("click", "#new_dir", function() {
      ...
      layer.prompt({ title: "请输入新文件夹名字", formType: 0 }, function(
        name,
        index
      ) {
        layer.close(index);
  ...
  fm.reload = function(id, options) {
    var config = getthisMmConfig(id); //获取当前实例配置项
    if (!config) return;

    var that = thisMm.that[id];
    that.reload(options);

    return thisMm.call(that);
  };
  //核心入口
  fm.render = function(options) {
    var inst = new Class(options);
    return thisMm.call(inst);
  };
  exports("mediaManager", fm);
});
